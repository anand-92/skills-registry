"""Tests for ``skills_mcp.__main__._first_paragraph``."""

from __future__ import annotations

from skills_mcp.__main__ import _first_paragraph
...
def test_skips_leading_heading_paragraph() -> None:
	...
	assert _first_paragraph(text) == "Actual body content here."


def test_collapses_internal_whitespace() -> None:
	text = "Line one\n   line two with   spaces\n\nnext"
	assert _first_paragraph(text) == "Line one line two with spaces"


def test_respects_limit_parameter() -> None:
	body = "x" * 500
	assert _first_paragraph(body, limit=10) == "x" * 10


def test_default_limit_is_240() -> None:
	body = "y" * 500
	assert len(_first_paragraph(body)) == 240
...
def test_only_headings_falls_back_to_truncated_text() -> None:
	...
	result = _first_paragraph(text, limit=50)
...
def test_paragraphs_separated_by_blank_lines() -> None:
	text = "alpha\n\nbeta\n\ngamma"
	assert _first_paragraph(text) == "alpha"


def test_leading_blank_lines_are_skipped() -> None:
	text = "\n\n\nfirst real line\n\nnext"
	assert _first_paragraph(text) == "first real line"
