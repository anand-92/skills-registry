...
func TestEnter_TriggersDownload(t *testing.T) {
	stub := &stubDownloader{dest: "/tmp/cache/skills-mcp/skills/foo_skill"}
...
func TestDownloadDoneMsg_Success(t *testing.T) {
	...
	got, _ := m.Update(downloadDoneMsg{
		slug: "foo_skill",
		dest: "/tmp/cache/skills-mcp/skills/foo_skill",
	})
	mm := got.(ListModel)
	...
	if !strings.Contains(mm.toast, "Foo") || !strings.Contains(mm.toast, "/tmp/cache/skills-mcp/skills/foo_skill") {
		t.Fatalf("toast = %q, want it to mention Foo and dest path", mm.toast)
	}
	if mm.rowDest["foo_skill"] != "/tmp/cache/skills-mcp/skills/foo_skill" {
