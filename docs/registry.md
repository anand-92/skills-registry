...
| `skills-registry` (npm) | Node | npm registry (`npx skills-registry`) | Thin launcher in [`npm/`](../npm). Ships no binary; its `postinstall` (lazy first-run fallback for `--ignore-scripts`) downloads the matching GitHub Release tarball/zip, extracts the binary, and execs it. Package version maps 1:1 to the release tag; published by `release.yml` when `NPM_TOKEN` is set. |
